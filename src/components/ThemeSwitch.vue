<template>
    <div class="switch-wrapper">
        <span>Light</span>
        <label id="switch"><input type="checkbox" v-model="checked" v-on:change="toggle" />
            <div></div>
        </label>
        <span>Dark</span>
    </div>
...
export default {
    ...
    methods: {
        toggle() {
            ...
            svg.select('.regions').selectAll('path').attr('fill', getComputedStyle(document.documentElement).getPropertyValue('--bg-04'));
        }
    },
}
</script>
